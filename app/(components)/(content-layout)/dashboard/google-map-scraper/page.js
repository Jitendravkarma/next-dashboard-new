...
const GoogleMapScraper = () => {
	...
	const recordData = [
		{ id: 1, icon: recordIcon, title:'records', class: "Total Records", text: 0, color: "primary/10" },
		{ id: 2, icon: websiteIcon, title:'website', class: "Total Website", text: 0, color: "primary/10" },
		{ id: 3, icon: emailIcon, title:'email', class: "Total Email", text: 0, color: "primary/10" },
		{ id: 4, icon: phoneIcon, title:'phone', class: "Total Phone", text: 0, color: "primary/10" },
	]
	...
	const csvHeaders = [
        ...
        { label: "Raviews", key: "reviews" },
        { label: "Latitude", key: "latitude" },
        { label: "Longitude", key: "longitude" },
        { label: "Map Link", key: "map" }
    ];
	...
	const convertData = (data)=>{
		return data.map(({title, website, address, description, category, timings, extra_services, latitude, longitude, avg_rating, reviews, phone_num, link}, ind)=>{
			return {
				id: ind+1,
				website : website || "N/A",
				title,
	...
	const startScraping =  async ()=>{
		if(queryBox.length){
			...
			const makeRequest = queries.map(async query=>{
				const makeRequest = await getGoogleMapData(query)
						...
						</div>
						{
							...
							<>
								...
								<div className="px-6 pb-4">
									{
										isActivated ?
										<Download csvHeaders={csvHeaders} data={data} fileName={"google-map-scrapper.csv"}/>
										:
