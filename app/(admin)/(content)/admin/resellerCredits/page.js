...
import { ContactBox, LimitReachedBox, SmsBox, AllocateKey, WhatsappBox } from "@/shared/layout-components/dashboard/AlertBox";
import Snackbar from "@/shared/layout-components/dashboard/SnackBar";

const UserAnalytics = () => {
    const { contactNum, smsNum, whatsAppNum, limitErr, openSnack, snackMessage, usersData, allocateKeyBoxClose, allocateKeyBox } = useUserContext()
    // const [ validity, setValidity ] = useState(false);
    // const [ userId, setUserId ] = useState(false);
    const [ data, setData ] = useState([]);
    const [userId, setUserId ] = useState('');
    const [whoClick , setWhoClick] =useState('');
    ...
    const columns = [
        ...
        {
            headerName: "Truecaller Credits",
            field: "Truecaller credits",
            width: 200,
            renderCell: (params) => {
  ...
  onClick={(e) => {
    ...
    setWhoClick(`Truecaller Credits`);
  }}
>Add Credits</button>
                </span>
                )
...
// 		},
    ...
    return (
        <div>
            ...
            <Seo title='Reseller Credits' />
            <PageHeader currentpage="Reseller Credits " img="/assets/img/users/profile.png" activepage="Admin" mainpage="Reseller Credits " />
            ...
            {
                allocateKeyBox &&
                <AllocateKey id={userId} type={whoClick} closeModel={()=>allocateKeyBoxClose()}/>
            }
            {/* alert boxes */}
        </div>
    );
