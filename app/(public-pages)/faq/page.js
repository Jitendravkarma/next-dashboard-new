      ...
      <section className="bg-white py-24">
        <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
          ...
          <div className="grid pt-8 text-left  md:gap-16  md:grid-cols-1">
            <div>
              {
                questions.map(({title, description, list}, ind)=>(
                  <FaqQuestion key={ind} title={title} description={description} list={list}/>
                ))
              }
            </div>  
            <div className="text-center">
                <Link href={'/signup'} target="_blank" className="p-3 text-sm uppercase bg-green-400 text-white rounded-sm hover:bg-green-500">
                  Start a Free Trial
                </Link>
            </div>      
          </div>
