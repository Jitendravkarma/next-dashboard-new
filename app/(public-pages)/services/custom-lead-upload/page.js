...
import { seo_custom_lead_upload } from "@/shared/data/seo-content/content";
import {
  customleaduploaddataget,
  customleaduploadProps,
  customLeadUploadStatus,
  customLeadUploadVideo,
  HowitworksCsvUploadCrm,
...
export const metadata = seo_custom_lead_upload;
    ...
    <>
      <section className="py-14 lg:py-16 bg-gradient-to-b from-blue-200 to-white flex justify-center items-center">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <PageHeader {...customleaduploadProps} />
          <div className="flex justify-evenly flex-wrap items-center gap-6 pt-12">
            {customLeadUploadStatus.map((item, index) => (
              <div
                key={index}
        ...
        <div className="w-full lg:w-[80%] text-center px-4 lg:px-16">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
            <div className="bg-white shadow-md rounded-md lg:px-8 px-2 py-10 col-span-1">
              {customleaduploaddataget
                .slice(0, Math.ceil(customleaduploaddataget.length / 3))
                .map((data, dataindex) => (
                  <div
                    key={dataindex}
            ...
            <div className="bg-white shadow-md rounded-md lg:px-8 px-2 py-10 col-span-1">
              {customleaduploaddataget
                .slice(
                  Math.ceil(customleaduploaddataget.length / 3),
                  Math.ceil((2 * customleaduploaddataget.length) / 3)
                )
                .map((data, dataindex) => (
                  <div
                    key={dataindex}
            ...
            <div className="bg-white shadow-md rounded-md lg:px-8 px-2 py-10 col-span-1">
              {customleaduploaddataget
                .slice(Math.ceil((2 * customleaduploaddataget.length) / 3))
                .map((data, dataindex) => (
                  <div
                    key={dataindex}
                    className="flex items-center justify-between md:justify-start text-base text-black font-light gap-2 mb-2"
      ...
      <HowItWork scraperData={HowitworksCsvUploadCrm} />
      <VideoSection {...customLeadUploadVideo} toolName={``} />
      <FaqSection />
      <HorizontalAds/>
      <WhyChooseUs />
    </>
