...
import { seo_indiamart_scraper } from "@/shared/data/seo-content/content";
import {
  indiamartscraperstatus,
  Howitworksindiamartscraper,
  indiamartscraperdataget,
  indiamartscraperProps,
  indiamartscraperVideo,
...
export const metadata = seo_indiamart_scraper;
const IndiaMartScraper = () => {
  return (
    <>
      <section className="py-14 lg:py-16 bg-gradient-to-b from-blue-200 to-white flex justify-center items-center">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="text-center px-4 lg:px-16">
            <PageHeader {...indiamartscraperProps} />
            <div className="flex justify-evenly flex-wrap items-center gap-6 pt-12">
              {indiamartscraperstatus.map((item, index) => (
                <div
                  key={index}
        ...
        <div className="w-full lg:w-[80%] text-center px-4 lg:px-16">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
            <div className="bg-white shadow-md rounded-md lg:px-8 px-2 py-10 col-span-1">
              {indiamartscraperdataget
                .slice(0, Math.ceil(indiamartscraperdataget.length / 3))
                .map((data, dataindex) => (
                  <div
                    key={dataindex}
            ...
            <div className="bg-white shadow-md rounded-md lg:px-8 px-2 py-10 col-span-1">
              {indiamartscraperdataget
                .slice(
                  Math.ceil(indiamartscraperdataget.length / 3),
                  Math.ceil((2 * indiamartscraperdataget.length) / 3)
                )
                .map((data, dataindex) => (
                  <div
                    key={dataindex}
            ...
            <div className="bg-white shadow-md rounded-md lg:px-8 px-2 py-10 col-span-1">
              {indiamartscraperdataget
                .slice(Math.ceil((2 * indiamartscraperdataget.length) / 3))
                .map((data, dataindex) => (
                  <div
                    key={dataindex}
                    className="flex items-center justify-between md:justify-start text-base text-black font-light gap-2 mb-2"
      ...
      <HowItWork scraperData={Howitworksindiamartscraper} />
      <VideoSection {...indiamartscraperVideo} toolName={`indiamart`} />
      <FaqSection />
      <HorizontalAds/>
      <WhyChooseUs />
    </>
