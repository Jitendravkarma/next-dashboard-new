...
import { seo_meta_ads } from "@/shared/data/seo-content/content";
import {
  metaadscrmdataget,
  metaAdsCrmProps,
  metaAdsCrmstatus,
  metaAdsCrmVideo,
  HowitworksMetaAdsCrm,
...
export const metadata = seo_meta_ads;
...
const BingSearch = () => {
  return (
    <>
        ...
        <div className="w-full lg:w-[80%] text-center px-4 lg:px-16">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
            <div className="bg-white shadow-md rounded-md lg:px-8 px-2 py-10 col-span-1">
              {metaadscrmdataget
                .slice(0, Math.ceil(metaadscrmdataget.length / 3))
                .map((data, dataindex) => (
                  <div
                    key={dataindex}
            ...
            <div className="bg-white shadow-md rounded-md lg:px-8 px-2 py-10 col-span-1">
              {metaadscrmdataget
                .slice(
                  Math.ceil(metaadscrmdataget.length / 3),
                  Math.ceil((2 * metaadscrmdataget.length) / 3)
                )
                .map((data, dataindex) => (
                  <div
                    key={dataindex}
            ...
            <div className="bg-white shadow-md rounded-md lg:px-8 px-2 py-10 col-span-1">
              {metaadscrmdataget
                .slice(Math.ceil((2 * metaadscrmdataget.length) / 3))
                .map((data, dataindex) => (
                  <div
                    key={dataindex}
                    className="flex items-center justify-between md:justify-start text-base text-black font-light gap-2 mb-2"
      ...
      <HowItWork scraperData={HowitworksMetaAdsCrm} />
      <VideoSection {...metaAdsCrmVideo} toolName={``} />
      <FaqSection />
      <HorizontalAds/>
      <WhyChooseUs />
    </>
