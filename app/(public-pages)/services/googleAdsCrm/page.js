...
import { seo_googleAdsCrm } from "@/shared/data/seo-content/content";
import {
  googleAdsCrmstatus,
  HowitworksGoogleAdsCrm,
  googleAdsCrmdataget,
  googleAdsCrmProps,
  googleAdsCrmVideo,
...
export const metadata = seo_googleAdsCrm;
...
const JustDialScraper = () => {
  return (
    <>
        ...
        <div className="w-full lg:w-[80%] text-center px-4 lg:px-16">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
            <div className="bg-white shadow-md rounded-md lg:px-8 px-2 py-10 col-span-1">
              {googleAdsCrmdataget
                .slice(0, Math.ceil(googleAdsCrmdataget.length / 3))
                .map((data, dataindex) => (
                  <div
                    key={dataindex}
            ...
            <div className="bg-white shadow-md rounded-md lg:px-8 px-2 py-10 col-span-1">
              {googleAdsCrmdataget
                .slice(
                  Math.ceil(googleAdsCrmdataget.length / 3),
                  Math.ceil((2 * googleAdsCrmdataget.length) / 3)
                )
                .map((data, dataindex) => (
                  <div
                    key={dataindex}
            ...
            <div className="bg-white shadow-md rounded-md lg:px-8 px-2 py-10 col-span-1">
              {googleAdsCrmdataget
                .slice(Math.ceil((2 * googleAdsCrmdataget.length) / 3))
                .map((data, dataindex) => (
                  <div
                    key={dataindex}
                    className="flex items-center justify-between md:justify-start text-base text-black font-light gap-2 mb-2"
      ...
      <HowItWork scraperData={HowitworksGoogleAdsCrm} />
      <VideoSection {...googleAdsCrmVideo} toolName={`googleAdsCrm`} />
      <FaqSection />
      <HorizontalAds/>
      <WhyChooseUs />
    </>
